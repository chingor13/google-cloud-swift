...
final class ReadObjectCoordinator: @unchecked Sendable {
  ...
  private func nextChunkImpl() async throws -> ByteChunk? {
    ...
    if options.effectiveRange.isZeroBytes {
  ...
  private func validateChecksumsAtEOF() throws {
    ...
    let isRangedRead = (options.effectiveRange != .entire)
    let isDecompressedTranscoding =
      (currentMetadata.storedContentLength != nil && currentMetadata.contentEncoding == nil)
    let checksums = options.effectiveChecksums

    if let crcOption = checksums.crc32c, let calc = crc32cCalculator {
    ...
    if let md5Option = checksums.md5, let calc = md5Calculator {
      let actual = calc.finalize()
      switch md5Option {
      case .auto:
  ...
  private func resumeDownload(underlyingError: Error) async throws {
    ...
    guard
      let resumeRange = calculateResumeRange(
        originalRange: options.effectiveRange,
